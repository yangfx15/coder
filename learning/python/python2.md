...
你可以反复调用`write()`来写入文件，但是务必要调用`f.close()`来关闭文件。当我们写文件时，操作系统往往不会立刻把数据写入磁盘，而是放到内存缓存起来，空闲的时候再慢慢写入。
...
完整路径表示：os.path.join('a', 'b') --> 'a/b'
创建目录：os.mkdir('')
删除目录：os.rmdir('')
获取文件扩展名：os.path.splitext('/path/file.py') --> ('/path/file', '.py')
文件重命名：os.rename('text.txt', 'text.py')
删除文件：os.remove('text.py')
获取目录下所有文件：
...
Python 的 dict 对象可以直接序列化为 JSON 的 {}，那怎么用 class  来序列化呢？
...
有两种方式，一种就是提供一个 `student2dict` 函数，一种是使用对象自带的属性：
...
Python 提供了 `multiprocessing` 来支持 Windows 的多进程调用：
...
Python解释器由于设计时有**GIL全局锁**，导致了多线程无法利用多核。多线程的并发在Python中就是一个美丽的梦。



### 5.3 ThreadLocal

多线程中的局部变量可以用 ThreadLocal 实现：
...
多进程的优点是稳定性高，因为一个子进程崩溃了，不会影响主进程和其他子进程。（当然主进程挂了所有进程就全挂了，但是Master进程只负责分配任务，挂掉的概率低）
...
计算密集型任务的特点是要进行大量的计算，消耗CPU资源。因此，代码运行效率至关重要。
...
Python语言，单线程的异步编程模型称为协程，有了协程的支持，就可以基于事件驱动编写高效的多任务程序。我们会在后面讨论如何编写协程。



### 5.5 分布式进程

在Thread和Process中，应当优选Process，因为进程更稳定，而且，Process可以分布到多台机器上，而Thread最多只能分布到同一台机器的多个CPU上。
