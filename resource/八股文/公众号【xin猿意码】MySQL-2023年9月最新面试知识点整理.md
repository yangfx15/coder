...
MySQL 的索引是一种用于提高数据库查询性能的数据结构。

它是一种数据结构，以有序的方式存储一列或多列的值，以便快速查找、排序和过滤数据。

索引可以大大减少数据库查询的时间复杂度，提高查询速度，尤其在大型数据表中效果显著。
...
- 组合索引：即多个数据列组成的索引，B+ 树会按照从左到右的顺序建立搜索树
...
![img](../../acticles/网盘系统/imgs/1667183181193-2e2c4137-e66c-45f1-a805-334c02061f29.png)
...
2. **叶子节点通过链表连接起来**，由于叶子节点的数据都是有序的，所以在范围查询的时候可以顺着链表往下走，就可以直接返回一段数据，进而减少 IO 次数，提升查询性能。
 ...
 MySQL 中，如果没有特别说明，**默认都是使用 B+ 树做索引的数据结构**。
...
即树高最高不超过 `2*log(n)`，因此它查找时间复杂度为 `log(n)`，无论是增删改查，红黑树的性能都比较稳定。
...
故在 MySQL 中，如果没有特别说明，**默认都是使用 B+ 树做索引的数据结构**。
...
**最左匹配原则**：当创建复合索引时，B+ 树按照从左到右的顺序建立搜索树，当通过组合索引（A，B，C）查询数据时，条件里面存在（AB）或者（AC），都可以走组合索引查询数据，但查询条件只有（BC）时，不会走组合索引。

一般，在条件允许的情况下，可将组合索引替代多个索引列使用。
...
事务并发可能导致脏读、重复读和幻读等问题。

#### 如何解决事务并发问题？

通过设置不同的隔离级别来避免数据出错，分别有 RU、RC、RR 和可串行化这四种。
...
通过加锁来实现事务的隔离性，确保多个事务不会同时修改相同的数据。

> 频繁的加锁可能会导致什么问题？

频繁的加锁可能降低数据库的读写性能，因为读取时不能修改数据，修改时不能读取数据。

> 数据库是如何解决加锁后的性能问题的？

通过MVCC（多版本并发控制）实现读取数据时不需要加锁，允许同时读取和修改数据，从而提高了事务的并发性能。



### MVCC 是什么

MVCC（Multi-Version Concurrency Control，多版本并发控制），在读取数据时通过一种类似**快照**的方式将数据保存下来，不同事务的 `session` 会看到自己特定版本的数据。
...
> 注意：`插入操作`的 undo 日志没有回滚指针， 因为它是新增的数据，没有老版本。
>
> 而已`删除`的信息会在 undo 日志记录的头信息中存一个 `delete flag` 标记，当该标记为 true 时， 表示已删除，则不返回数据。



#### 版本链

下图是一个简洁的版本链图示，InnoDB 中的 **undo 日志保存的就是一个版本链**：
...
假设当前 select 的 trx_id 为 102，read-view 中未提交的事务为 [101]，102 比 101 大，所以该版本的数据不可见【102之前没做过更新操作，所以不存在 read-view 中】。

这时，需要通过 DB_ROLL_PTR 获取上一版本的 trx_id 100。
...
当 SQL 通过主键查询时，若 where 语句全部命中，则只会加记录锁。比如以下示例：

Session A : 若数据库存在 normal_key= 9 的这一条数据，delete from table where normal_key= 9; 

Session B：由于 A where条件全部命中，normal_key= 10 的记录并没有给锁住，insert into table values(10, abc);



前文已经说过了，gap 间隙锁是行锁的一种，事务加锁后其锁住的是表记录的一个区间，即表记录相邻 ID 之间出现的空隙。
...
// gap 锁边际：
插入【6】 // 成功 
因为 normal_key = 6 对应 primary_key = c; 
bb 首字母排序 < c 可以在(6,c)的左边插入
insert into table values(6,bb);  // 失败 
因为 normal_key = 6 对应 primary_key = c; 
dd 首字母排序 > c 只能在(6,c)的右边插入 
...
Session A : // 若 数据库存不存在这条数据 delete from table where normal_key= 7; 

Session B： // 由于 A where条件全部未命中，整个表给锁住 
...
使用双节点数据库，搭建单向或者双向的半同步复制。在 MySQL5.7 以后的版本中，由于 lossless replication、logical 多线程复制等一些列新特性的引入，使得 MySQL 原生半同步复制更加可靠。

常见架构如下：

![image-20230921092312548](imgs/image-20230921092312548.png)
...
![image-20230921092625819](imgs/image-20230921092625819.png)
...
![image-20230921092936313](imgs/image-20230921092936313.png)
