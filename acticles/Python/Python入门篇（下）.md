...
随着近年来，互联网技术的飞速发展，再加上移动端应用的突飞猛进，可以说真正让 web 站在时代的风口，在飞速前行。

Web 开发从应用产品的角度上，可分为以下四种。
...
使用到的开发语言有 C、C++、Java、C# 等，在桌面应用程序开发过程中，我们常常需要考虑的是语言的跨平台性以及软件的多平台支持，比如常见的操作平台 Windows、Linux、MacOS。我们开发一款桌面应用软件，一般需要考虑对这几类操作系统的支持。
...
这样，我们第一个用 FastAPI 启动的服务器就成功运行起来了，是不是非常简便。
...
这样，我们在传入参数时，可以先看接口文档，防止数据结构或字段格式出错。



更多资料，可参见 FastAPI 中文参考文档：https://fastapi.tiangolo.com/zh/



## 5. 小结

到这期为止，Python 入门篇就大概结束了。我们先是从 Python 的语言特性，基本数据类型到 Python 常用操作，到本期内容的 Web 开发，相信一起走完的同学可以开始上手工作了，后面，我们会结合 Python 语言的数据处理能力，一起和大家学一些 GitHub 上的高 Star 项目。比如股票处理，游戏开发，还有其它和 AI 大模型相关的应用。敬请期待！