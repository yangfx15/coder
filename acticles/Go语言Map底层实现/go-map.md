...
比如，之前在面试 “腾讯TEG” 部门一面时，视频会议对面坐着一位儒雅随和的面试官问到，“你平常是用 go 语言是吧，那说一下 go 语言中有哪些引用类型呢？”
...
但 go 语言中 map 的实现机制不算很复杂，且和别的语言在设计上有些许共同点，这我还是了解一点的。



## 2. map 的底层数据结构

于是，我从 map 的数据结构、增删以及扩容操作上几个层面上描述 map 的底层实现。
...
Go 语言的 map 是由 hmap 结构实现的，hmap 里记录了元素个数、桶个数[2^B]、以及 map 桶的地址（buckets），和溢出桶的地址（extra）。
...
buckets 记录了存放 map 桶的 bmap 地址，bmap 是记录 map 实际数据的关键结构，它在未编译时的结构体如下：
...
data 参数存储了真实的 map 数据（key/value），每个 bmap 存储 8 个 k-v 键值对，且键值对的存储方式是**分开连续存储**，如下图所示：

![image-20230303092536443](image\bmap.png)



当 k-v 对超过 8 个后，会新生成一个 bmap 存放数据，由 overflow 记录新 map 的地址。

在 go 的 map 实现中，底层结构体是 hmap，hmap 里维护着若干个 bucket 数组（桶数组）。为了直观理解，我们提炼 hmap 中的关键字段为图形模式：

![image-20230303092912960](image\hmap.png)

如上图所示，buckets 数组中每个元素都是 bmap 结构，即**每个 bucket 都是 bmap 结构**，每个 bmap 桶保存了 8 个 k-v 对，超过 8 个则新生成一个 bmap，由 overflow 记录新 bmap 的地址。
...
假设当前 B=4，即桶数量为 2^B = 16 个，要从 map 中获取 key5 对应的 value
...
2. 通过**最后 B 位来确定在哪号桶**，0100 转为十进制为 4，所以在 4 号桶；

3. 根据 hash 值的前 8 位和 tophash 的前 8 位比对，快速确定在桶的哪个位置，执行下一步；

4. 将查询的 key5 和 bmap 中的 key5 进行比对，如果完全匹配则获取对应 value；
...
![image-20230303164227974](image\put-map.png)

map 赋值可分为以下几步（假设我们添加 key6）：

- 通过 key6 的 hash 值后 "B" 位确定是哪一个桶；
- 遍历当前桶，通过 key 的 tophash 和 hash 值对比，防止 key 重复，然后找到第一个可以插入的位置，即**空位置处存储数据**；
- 如果当前桶元素已满，会创建一个新的桶并放入 key-value，然后通过 overflow 连接两个 bmap。



**关于 hash 冲突**：当两个不同的 key 落入同一个桶中时，就是发生了哈希冲突。当桶未满时，用开放寻址法从前往后找到第一个空位进行插入；当 bmap 桶已经有 8 个 k-v 键值对了，就会新建溢出桶（bmap），并通过 overflow 指针将其链接起来。
...
1. 如果删除的元素是值类型，如 int、float、bool、string 以及数组，map 的内存不会自动释放；
2. 如果元素是引用类型，如指针、slice、map 和 chan 等，map 的内存会自动释放，但释放的内存是子元素引用类型的内存占用；
3. 将 map 置为 nil 以后，内存释放，下一次 GC 时内存会被回收。
...
其中，Alloc 为当前堆对象占用的内存空间，单位是KB；TotalAlloc是累计分配的堆对象空间，Sys是占用操作系统的内存空间，NumGC是垃圾回收的次数。
...
![image-20230303171633438](image\equal.png)

由于 map 不断 put 和 delete 元素，因此桶中可能会出现很多断断续续的空位，这些空位会导致 bmap 溢出桶很长，导致扫描时间变长。所以，这种扩容实际上是一种整理，把后置位的数据整理到前面。**等量扩容时，元素会发生重排，但不会换桶，且 bmap 桶的个数也不会增加。**
...
如图所示，扩容前 B=2，扩容后 B=3，假设一个元素 key 的 hash 值后三位为 101，那么由上文介绍可知，扩容前，hash 值的后两位来决定几号桶。即**扩容前， 01 在 1 号桶，扩容后 101 在 5 号桶，发生了迁移**。
...
从公式可得，**装载因子是指当前 map 中，每个桶的平均元素个数**。

**扩容条件1：装载因子 > 6.5**

正常情况，如果没有溢出桶，那么一个桶中最多有 8 个元素，当平均每个桶中的元素超过 6.5 个时，就意味着当前容量快满了，需要扩容。

**扩容条件2：溢出桶的数量过多**

当 B < 15 时，如果 overflow 的 bmap 数量达到 2^B；

当 B >= 15 时，overflow 的 bmap 数量达到 2^15。

简单来讲，新加入 key 的 hash 值后 B 位都一样，使得个别桶一直在插入新数据，进而导致它的溢出桶链条越来越长。这样一来，在 map 操作数据时，扫描速度就会越来越慢。及时扩容，可以对这些元素重排，使得元素在桶的位置更平均一些。
...
2. 增量扩容非全量扩容（防止卡顿），每次对 map 进行删改时，会触发从 oldbuckets 迁移到 buckets 的操作；
...
**不可对元素取址：随着 map 元素的增长，map 底层可能会重新分配空间，导致之前的地址无效**；
...
**线程不安全：用 Go 自带的 sync.RWMutex 锁来保证，或者使用 sync.Map（可支持并发锁和共享锁）**

举个栗子，在同一时间点，两个 goroutine （比如 g1 和 g2）对同一个 map 进行读写操作。

某 map 桶数量为 4，即 B=2，当前元素数量也为 4，桶满了。此时，g1 插入 key1，g2 读取 key2，可能会发生以下：

- g2 计算 key2 的 hash 值，B=2，并确定桶号为1；
- g1 添加 key1，触发扩容条件；
- B=B+1=3，buckets 数据迁移到 oldbuckets，扩容完成；
- g2 从桶 1 中遍历，获取数据失败！

想要迎战互联网大厂的技术面，对 go 里面的数据类型不仅得应用广泛，还得在底层机制的认知上花一番功夫。毕竟，八股文谁都会背，但在实际应用中如果不了解数据结构的底层实现，是很难写出高质量的代码滴~
...
掘金：https://juejin.cn/post/7029679896183963678#heading-9

知乎：https://zhuanlan.zhihu.com/p/495998623
